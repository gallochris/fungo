...
pb <- odds %>% 
  rvest::html_nodes('table') %>% 
  xml2::xml_find_all("//td[contains(@class, 'goto pb _pb odds-cell null')]") %>% 
  rvest::html_text()
...
foxbet <- odds %>% 
  rvest::html_nodes('table') %>% 
  xml2::xml_find_all("//td[contains(@class, 'goto fb _fb odds-cell null')]") %>% 
  rvest::html_text()


chart_df <- data.frame(team, pb, dk, foxbet)

kable(chart_df, col.names = c("Team",
                           "PointsBet",
                           "DraftKings",
                           "FoxBet"), caption = "NBA Champ Odds", format="markdown")
